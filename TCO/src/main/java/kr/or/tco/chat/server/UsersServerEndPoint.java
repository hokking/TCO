			...
			if (connectingUser != null && !username.equals(connectingUser)) {
				...
				for (Session session : connectedAllUsers) {
					if (connectingUser.equals(session.getUserProperties().get("username"))) {
						LOGGER.info(username + " chatroomMembers is entered.");
				...
				for (Session session : chatroomMembers) {
					...
					session.getBasicRemote().sendText(
							...
							.add("enterChatId", chatroomId)
							.add("username", (String) session.getUserProperties().get("username"))
							.build().toString());
					LOGGER.info(username + " for chatroomMembers is entered.");
				}
			}
		}
	}
